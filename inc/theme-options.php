...
function theme_options_init() {
	register_setting('tarvo_options', 'tarvo_theme_options', 'theme_options_validate');
}
...
function theme_options_add_page() {
	add_theme_page(__('Tarvo Theme Options', 'tarvo'), __('Tarvo Theme Options', 'tarvo'), 'edit_theme_options', 'theme_options', 'theme_options_do_page');
}
...
function theme_options_do_page($tab = '') {
	...
	<div class="wrap">
		<div class="tarvo-optionen">  <!-- begin: .tarvo-optionen -->
			<?php 
			echo "<h2>" . wp_get_theme() . ': ' . __('Options', 'tarvo') . "</h2>";
			?>

			<?php if (false !== $_REQUEST['settings-updated']) : ?>
				<div class="updated fade"><p><strong><?php _e('Options updated.', 'tarvo'); ?></strong></p></div>
				<?php
			endif;
			...
			if (!isset($setoptions['tarvo_theme_options'][$tab])) {
				echo "Invalid Tab-Option or undefined Option-Field $tab";
			}


			echo "<h3 class=\"nav-tab-wrapper\">\n";
			foreach ($setoptions['tarvo_theme_options'] as $i => $value) {
	...
	<?php settings_fields('tarvo_options'); ?>
				<input type="hidden" id="tarvo_theme_options[tab]" name="tarvo_theme_options[tab]"  value="<?php echo $tab; ?>">


				<div id="einstellungen">
					<table>
						<?php
						if (isset($setoptions['tarvo_theme_options'][$tab]['fields'])) {
							foreach ($setoptions['tarvo_theme_options'][$tab]['fields'] as $i => $value) {
								$name = $i;
								...
								if ($type == 'section') {
									...
									if ($type == 'bool') {
										echo "\t\t\t";
										echo "<input id=\"tarvo_theme_options[$name]\" name=\"tarvo_theme_options[$name]\"
                                            type=\"checkbox\" value=\"1\" " . checked($options[$name], 1, false) . ">\n";
										echo "\t\t\t";
										echo "<label for=\"tarvo_theme_options[$name]\">$label</label>\n";
									} elseif (($type == 'text') || ($type == 'email')) {
										echo "\t\t\t";
										echo "<input class=\"regular-text\" id=\"tarvo_theme_options[$name]\"
                                            type=\"text\" name=\"tarvo_theme_options[$name]\"
					    value=\"";
										...
										echo "\t\t\t";
										echo "<input class=\"large-text\" id=\"tarvo_theme_options[$name]\"
                                            type=\"text\" name=\"tarvo_theme_options[$name]\"
					    size=\"120\" value=\"";
										if (isset($options[$name]))
											echo esc_attr($options[$name]);
					    ...
					    size=\"120\" value=\"";
										...
										echo "\t\t\t";
										echo "<textarea class=\"large-text\" id=\"tarvo_theme_options[$name]\"
                                            cols=\"30\" rows=\"10\"  name=\"tarvo_theme_options[$name]\">";
										if (isset($options[$name]))
											echo esc_attr($options[$name]);
										echo "</textarea><br>\n";
										echo "\t\t\t";
										...
										echo "\t\t\t";
										echo "<input class=\"number\" size=\"5\" id=\"tarvo_theme_options[$name]\"
                                            type=\"text\" name=\"tarvo_theme_options[$name]\"
										...
										foreach ($liste as $option) {
											...
											<label class="plakattile">
												<div style="height: 40px; width: 100%; margin:0 auto; background-color: #F28900; color: white; display: block;">
													<input type="checkbox" name="tarvo_theme_options[<?php echo $name ?>][]"
								...
								<?php echo $sub ?>
										...
										foreach ($liste as $entry => $listdata) {
											...
											<div style="display: inline-block; width: 90%;" class="<?php echo $name ?>">
												<label for="tarvo_theme_options[<?php echo $name ?>][<?php echo $entry ?>][active]" class="<?php echo $entry ?>" style="width: 180px; display: inline-block;">
													<input type="checkbox"
														   id="tarvo_theme_options[<?php echo $name ?>][<?php echo $entry ?>][active]"
														   name="tarvo_theme_options[<?php echo $name ?>][<?php echo $entry ?>][active]"
														   value="1" <?php echo $checked; ?>>
						<?php echo $liste[$entry]['name'] ?>
												</label>

												<input id="tarvo_theme_options[<?php echo $name ?>][<?php echo $entry ?>][content]"
													   type="text" name="tarvo_theme_options[<?php echo $name ?>][<?php echo $entry ?>][content]"
										...
										echo "\t\t\t";
										echo "<select name=\"tarvo_theme_options[$name]\">\n";
				...
				<p class="submit">
					<input type="submit" class="button-primary" value="<?php echo __('Save Options', 'tarvo'); ?>" />
				</p>
			</form>
		</div>

	</div> <!-- end: .tarvo-optionen -->
	<?php
}
...
function theme_options_validate($input) {
	...
	$saved = (array) get_option('tarvo_theme_options');
	//    $options= $saved;
